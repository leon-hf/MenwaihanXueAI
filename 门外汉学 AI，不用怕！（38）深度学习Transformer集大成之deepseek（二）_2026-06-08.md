...
$$
\langle \text{RoPE}(Q, m), \text{RoPE}(K, n) \rangle = \langle Q, K \rangle \cdot e^{i(m-n)\theta}
$$
这个公式的核心不是"给每个词加一个位置向量"，而是让第 $m$ 个位置的Q向量旋转 $m$ 对应的角度，让第 $n$ 个位置的K向量旋转 $n$ 对应的角度。当两者做内积时，绝对位置会自然转化成相对位置差 $n-m$。


![](images/第38篇/img_02.jpg)
...
### 分组策略
* 实现方式：将高维向量拆分为多个二维子空间，每组独立旋转。
* 通俗理解：类似多齿轮联动，每个齿轮负责不同频率的信息。例如前32组高频齿轮处理主谓一致，后224组低频齿轮分析篇章结构。
...
### 案例解析：句子"小明打了小强，但自己也被老师批评了"

假设词向量维度为512维（前64维高频，后448维低频）：

**高频维度（如第0-63维）**
主要负责捕捉"小明"和"打"的**主谓关系**（位置0和1），"打"和"小强"的**动宾关系**（位置1和2）。

* 高频的正余弦波，如维度0，波长极短，相邻位置，如位置0到1的编码值剧烈变化
* 当计算"小明（位置0）"和"打（位置1）"的注意力时，高频维度的相位差大，模型能立刻感知这是**紧邻的主谓结构**

**低频维度（如第64-511维）**
主要负责理解"小明被批评"与前半句的**因果逻辑**（相隔5个词），以及整句话的**教育主题**

* 低频的正余弦波，如维度500，位置0->5的编码值变化缓慢
...
$$
\theta_d' = \theta_d \cdot b'^{-2d/|D|} \quad (b' = b \cdot s^{\frac{|D|}{|D|-2}})
$$
### NTK-by-parts分段优化
**问题2：时针分针协同失调，即token之间的相对位置关系混乱**
...
* 高频/短波长维度：尽量不插值或轻度缩放，保留近距离分辨率。
* 低频/长波长维度：进行更强的插值或缩放，支持更长距离的位置表达。
* 中频维度：使用平滑过渡函数，避免高频和低频之间出现断层。
...
* **当 0 < T < 1**：概率分布会变得更"尖锐"，高概率token的概率被放大，低概率token的概率被抑制，模型输出更加确定和保守。

* **当T > 1（高温）**：概率分布会变得更"平滑"，拉高低概率token的相对概率，从而增加输出的随机性和创造性，但也可能带来不连贯的风险。

* **当T = 1**：为标准softmax，不改变原始分布 。
