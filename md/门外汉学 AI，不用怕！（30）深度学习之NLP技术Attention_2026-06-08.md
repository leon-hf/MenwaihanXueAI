...
传统编码器采用单向RNN/LSTM结构，仅能捕捉前向依赖关系。当处理**倒装句或逻辑嵌套结构**时，如中文倒装句“所以不吃鱼，他因为过敏”，模型因无法反向关联后置的否定原因：“过敏” ，而错误生成肯定语义：“他吃鱼”。
...
* 信息解压缩：摒弃seq2seq中的Context-Vector单一上下文向量，改为存储编码器所有时间步的隐藏状态，保留完整输入信息；
* 动态权重分配：解码器生成每个目标词时，计算其与编码器各位置的关联权重，实现“按需聚焦”；
* 双向感知增强：结合双向编码器（BiRNN），同时捕捉前向与反向语义依赖。
...
![](images/第30篇/img_04.png)


计算步骤：  
1. 相似度计算：用解码器的当前隐藏状态（如生成“does not eat”时的状态）和编码器的每个隐藏状态（如“过敏”）计算相似度，得分越高表示相关性越强。  相似度的计算公式为：
   $$
   \text{对齐分数} = \text{重要性权重}^\top \times \tanh\big( \text{解码器投影矩阵} \times \text{解码器上一状态} + \text{编码器投影矩阵} \times \text{编码器隐藏状态} \big)
   $$
   又写作：
   $$
   e_{ij} = \mathbf{v}^\top \tanh(\mathbf{W}_a \mathbf{s}_{i-1} + \mathbf{U}_a \mathbf{h}_j)
   $$
2. 权重归一化：通过Softmax将相似度分数转化为概率分布，其总和为1。例如，“过敏”和“不吃鱼”可能占90%的权重。  
3. 加权求和：用权重对编码器的隐藏状态加权求和，最终生成动态上下文向量。公式简化：
$$
\text{注意力权重} = \text{Softmax}(\text{解码器状态} \cdot \text{编码器隐藏状态}) \\ 
$$
<br/>

$$
\text{上下文向量} = \sum (\text{注意力权重} \times \text{编码器隐藏状态})
$$
## 4）注意力机制运用过程
### 1）编码阶段：全面捕获输入语义
输入中文句子：“他因为对鱼过敏，所以不吃鱼”，通过双向RNN进行编码；

* 前向RNN从左到右处理每个词，捕获“左→右”的上下文：“他→因为→对鱼过敏”，生成前向隐藏状态 $\vec{h}_j$；
* 后向RNN从右到左处理，捕获“右→左”的上下文：“不吃鱼→所以→对鱼过敏”，生成后向隐藏状态 $\overleftarrow{h}_j$；
...
\text{对齐分数} = \text{重要性权重}^\top \times \tanh\big( \text{解码器投影矩阵} \times \text{解码器上一状态} + \text{编码器投影矩阵} \times \text{编码器隐藏状态} \big)
$$
对所有得分用softmax归一化，得到注意力权重 $\alpha_{t,j}（\sum_j \alpha_{t,j}=1）$，权重越高表示当前解码步骤越需要关注输入序列的第 $j$ 个词。
...
\text{注意力权重} = \text{Softmax}(\text{解码器状态} \cdot \text{编码器隐藏状态}) \\ 
$$
该向量是“定制化”的：即每个目标词对应一个专属 $\mathbf{c}_t$ 上下文向量，只聚合当前最相关的输入语义，而不是整句共享一个固定向量。
...
（1）中文按语义分词为：
* [他, 因为, 对鱼过敏, 所以, 不吃鱼]（5个token，实际不止）

（2）英文分词为：
* [He, does not eat fish, because, he, is allergic to them, </s>]（6个token+结束符）；
...
* 输入：例句的中文词嵌入序列[“他”的嵌入, “因为”的嵌入, “对鱼过敏”的嵌入, “所以”的嵌入, “不吃鱼”的嵌入]；

* 双向编码过程：
  * 前向LSTM：从左到右处理，生成 $\vec{h}_1$（“他”+左上下文）、$\vec{h}_2$（“因为”+“他”的前置语境）、$\vec{h}_3$（“对鱼过敏”+“因为”的前置语境）、$\vec{h}_4$（“所以”+“对鱼过敏”的前置语境）、$\vec{h}_5$（“不吃鱼”+“所以”的前置语境）；
  * 后向LSTM：从右到左处理，生成 $\overleftarrow{h}_1$（“他”+“因为对鱼过敏”的后置语境）、$\overleftarrow{h}_2$（“因为”+“对鱼过敏+所以不吃鱼”的后置语境）、$\overleftarrow{h}_3$（“对鱼过敏”+“所以不吃鱼”的后置语境）、$\overleftarrow{h}_4$（“所以”+“不吃鱼”的后置语境）、$\overleftarrow{h}_5$（“不吃鱼”+右上下文）；
```
输入句子：
[他] [因为] [对鱼过敏] [所以] [不吃鱼]
...
h5 = [h5→ ; h5←]
```
* 输出：每个token的最终隐藏状态为双向拼接，即 $h_1=[\vec{h}_1;\overleftarrow{h}_1]$（“他”的完整语义）、$h_2=[\vec{h}_2;\overleftarrow{h}_2]$（“因为”的因果逻辑语义）、$h_3=[\vec{h}_3;\overleftarrow{h}_3]$（“对鱼过敏”的核心语义）、$h_4=[\vec{h}_4;\overleftarrow{h}_4]$（“所以”的转折语义）、$h_5=[\vec{h}_5;\overleftarrow{h}_5]$（“不吃鱼”的结果语义），形成编码器隐藏状态序列 $\mathbf{H}=[h_1,h_2,h_3,h_4,h_5]$。

这个过程的重点，不是简单“读两遍句子”，而是让每个位置都同时拥有**左边上下文**和**右边上下文**。 比如“对鱼过敏”这个位置，在双向编码之后，不再只知道前面有“因为”，也知道后面有“所以不吃鱼”。
```
“对鱼过敏”这个位置：
...
* **步骤1：初始化解码器状态。**
  * 用编码器双向最终状态（$\vec{h}_5+\overleftarrow{h}_1$） 拼接，得到解码器初始隐藏状态 $\mathbf{s}_0$，加载例句全局语境；
```
编码器读完整句后，手里有：
h1, h2, h3, h4, h5
...
* **步骤2：生成第一个英文token（“He”）。**
  * 注意力计算：以 $\mathbf{s}_0$ 为查询向量，通过加性评分函数 $e_{1,j}=\mathbf{v}_a^T \tanh(\mathbf{W}_a \mathbf{s}_0 + \mathbf{U}_a h_j)$ 计算与 $\mathbf{H}$ 中所有 $h_j$ 的相关性，此时 $e_{1,1}$（与 $h_1$ “他”的得分）最高，softmax后 $\alpha_{1,1}=0.88$（其他权重≤0.04），此时模型会更容易把注意力集中到主语“他”对应的位置上，进而输出“He”。
  * 上下文向量：$\mathbf{c}_1 = 0.88h_1 + 0.03h_2 + ... + 0.02h_5 ≈ h_1$，仅聚合“他”的语义；
  * 生成token：结合嵌入、$\mathbf{c}_1、\mathbf{s}_0$ 输入解码器GRU，更新状态为 $\mathbf{s}_1$，softmax输出“ He”；
```
当前要生成的词：He
...
He
```
* **步骤3：生成第二个英文token（“does not eat fish”）。**
  * 注意力计算：以 $\mathbf{s}_1$ 为查询向量，$e_{2,5}$（与 $h_5$“不吃鱼”的得分）最高，$\alpha_{2,5}=0.91$，模型聚焦“不吃鱼”（英文结果前置）；	当当前状态更需要“结果动作”这部分信息时，模型会更多关注“不吃鱼”相关的输入表示；
  * 上下文向量：$\mathbf{c}_2 ≈ h_5$，聚合“结果”语义；
  * 生成token：输出“does not eat fish”；
```
当前已经生成：
He
...
* **步骤4：生成第三个英文token（“because”）。**
  * 注意力计算：以 $\mathbf{s}_2$ 为查询向量，$e_{3,2}$（与 $h_2$ “因为”的得分）最高，$\alpha_{3,2}=0.85$ ，模型跨位置对齐中文“因为”（适配英文原因后置的倒装逻辑）；
  * 上下文向量：$\mathbf{c}_3 ≈ h_2$，聚合“原因引导”语义；
  * 生成token：输出“because”；
```
当前已经生成：
...
* **后续步骤：**生成“he”（聚焦 $h_1$）、“is allergic to them”（聚焦 $h_3$ “对鱼过敏”），直至生成结束符</s>。
```
当前已经生成：
...
### 3、损失计算：双重惩罚“对齐错误+翻译错误”
* 损失函数：交叉熵损失，计算生成序列与目标序列的概率差异；
* 损失聚焦：
  * 若模型出现“对齐错误”（如生成“because”时关注 $h_4$ “所以”），则 $\alpha_{3,4}$ 升高，“because”生成概率降低，损失值飙升；
  * 若出现“翻译错误”（如“对鱼过敏”译为“dislikes fish”），对应位置损失也会显著增大——损失函数同时约束“对齐精度”和“翻译准确性”。

* 学习机制：
  * 训练时通常并不会额外给出一份“标准注意力答案”，而是通过最终翻译结果的误差，把梯度反向传回注意力模块，让注意力权重也一起被优化；
  * 也就是说，注意力机制虽然学会了更合理的对齐，但它通常是通过翻译目标间接学出来的，而不是显式告诉模型“because 就必须对齐到第几个词”。

### 4、反向传播与参数优化（全参数联合更新）
* 梯度计算：从损失值出发，通过链式法则反向传播梯度，覆盖所有可学习参数（编码器/解码器LSTM/GRU权重、注意力层 $\mathbf{W}_a/\mathbf{U}_a/\mathbf{v}_a$、词嵌入矩阵、输出层权重）；

* 优化目标：调整注意力参数，让下次生成“because”时 $\alpha_{3,2}$ 更高、$\alpha_{3,4}$ 更低；调整解码器参数，让“对鱼过敏”精准映射为“is allergic to them”；

* 优化器：采用Adadelta（论文指定），自适应调整学习率，避免梯度消失，确保模型稳定收敛。

### 5、迭代训练：直至“倒装对齐精准+翻译通顺”

重复上述循环数万次，模型经历三个阶段：

* 初期（前1万次迭代）：对齐混乱（生成“because”时可能关注 $h_4$ “所以”）、翻译错误（“对鱼过敏”译为“hates fish”），损失值高；

* 中期（1-3万次迭代）：注意力权重逐渐精准，生成“because”稳定关注 $h_2$ “因为”，翻译错误减少，损失持续下降；

* 收敛后（3万次+迭代）：模型对例句稳定输出“He does not eat fish because he is allergic to them”，且能泛化到同类倒装句翻译（如“她因为怕黑，所以开灯”→“She turns on the light because she is afraid of the dark”）。
